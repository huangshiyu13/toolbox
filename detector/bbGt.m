...
if(isempty(dtDir)), 
    ...
    gtFs=myGetFiles(gtDir,'*.txt');
    dtFs=myGetFiles2(dtDir,'*.txt',gtDir);
end

% load ground truth
persistent keyPrv gtPrv; key={gtDir,pLoad}; n=length(gtFs);
if(isequal(key,keyPrv)), gt0=gtPrv; else gt0=cell(1,n);
  for i=1:n, gt0{i}=acfbbload(gtFs{i},pLoad); end
  gtPrv=gt0; keyPrv=key;
end
...
function out = myGetFiles( dirName, houzhui)
    ...
    for i = 1:len
        ...
        k = k+1;
        out{k} = [dirName '/' files(i).name];
    end
end

function out = myGetFiles2( dirName, houzhui,searchArea)

    if(nargin<2),
        files = dir(fullfile(searchArea));
    else
        files = dir(fullfile(searchArea,houzhui));
    end
